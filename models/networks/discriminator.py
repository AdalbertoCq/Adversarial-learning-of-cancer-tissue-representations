...
def discriminator_resnet_mask_class(images, layers, spectral, activation, reuse, init='xavier', regularizer=None, normalization=None, attention=None, down='downscale', label=None, name='discriminator', softmax=tf.constant(1.)):
	...
	with tf.variable_scope(name, reuse=reuse):
		...
		# class_logits = tf.cond(softmax > 0.5, lambda: tf.nn.log_softmax(class_logits), lambda: class_logits)
...
def discriminator_resnet_mask_class_tran(images, layers, spectral, activation, reuse, init='xavier', regularizer=None, normalization=None, attention=None, down='downscale', label=None, name='discriminator'):
	net = images
	channels = [32, 64, 128, 256, 512, 1024]
	...
	with tf.variable_scope(name, reuse=reuse):
		...
		# Dense.
		net = dense(inputs=feature_space, out_dim=channels[-2], spectral=spectral, init=init, regularizer=regularizer, scope=2)				
		net = activation(net)
